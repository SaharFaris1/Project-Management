...
function RegisterStu() {
  ...
  const btnSubmit = async (e) => {
    ...
    if (!email.includes("tuwaiq")) {
      Swal.fire("Error", "Email must contain 'tuwaiq'", "error");
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(apiUrl, {
        ...
        email,
        password,
        role: "student" //   إضافة الدور
      });

      Swal.fire("Success", "Registration successful", "success");

      //  حفظ المستخدم ودوره في localStorage
      localStorage.setItem("user", JSON.stringify({
        ...response.data,
        role: "student"
      }));

      navigate("/loginStu"); // تحويل الطالب إلى صفحة الدخول
    } catch (err) {
      console.error(err);
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="w-full max-w-md p-6 bg-white rounded shadow">
        <div className="text-center flex flex-col justify-center items-center mb-6">
          ...
          <h2 className="text-2xl font-bold mt-4">Student Registration</h2>
        </div>
        ...
        <form onSubmit={btnSubmit} className="space-y-4">
          ...
          <p className="mt-4 text-sm text-gray-600 text-center">
            ...
            <Link to="/loginStu" className="text-orange-500 hover:underline">
              Login
            </Link>
          </p>
        </form>
